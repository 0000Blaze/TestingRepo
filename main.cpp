...
int main(int argc, char ** argv)    //default arguments of main
{
    ...
    mainWindowIndex = glutCreateWindow("The Scholar");    //defines the name of the window
...
void callBackFun()    //Call  back function to draw on the window
{
    ...
    else if(pageNo==READING_P)
    {
        navigate.readingScreen();
        glutSwapBuffers();
    }
    else if(pageNo==COMPLETED_P)
    {
        navigate.completedScreen();
        glutSwapBuffers();
    }
    else if(pageNo==FAVOURITE_P)
    {
        navigate.favouriteScreen();
        glutSwapBuffers();
    }
    else if(pageNo==SHARED_P)
    {
...
namespace action
{
    void loginButtonPressed(logIn login)
    {
        ...
        {
            pageNo=READING_P;               //to reading page
...
namespace logInN
{
    ...
    void mouse(int x,int y,logIn login)
    {
        ...
        if((x>=300 && x<=450) && (y>=440 && y<=495)) //login page to reading page
        {
...
namespace ReadingScreen
{
    ...
    void mouse(int button,int state,int x,int y)
    {
        ...
        else if((x>=580 && x<=670) && (y>=65 && y<=100)) //from reading page to completed page
        {
            bossel=3;
            pageNo=COMPLETED_P;
        }
...
void keyPressed(unsigned char pressedKey,int x,int y)
{
    ...
    {
       // ReadingScreen::KeyBoard(pressedKey,x,y);
    }
...
void mouseclicked(int button,int state,int a,int b)
{
    ...
    if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
    {
        ...
        {
            ReadingScreen::mouse(button,state,x,y);
        }
        /*else if(pageNo==TODO_P)
        {
            Todo::mouse(x,y);
...
void windows::readingScreen()
{
    ...
    glDrawP(9.5,7,-3,-1);
    printText(6.9,7.3,"Completed",GLUT_BITMAP_8_BY_13,0,0,0);
    //SearchWindow::makeObjects();
}

void windows::completedScreen()
{
    glRasterPos2f(-2,9);
    printText(-2.25,8.8,("Hi,"+userName).c_str(),GLUT_BITMAP_HELVETICA_18,1,1,0);
    glColor3f(1,1,1);
